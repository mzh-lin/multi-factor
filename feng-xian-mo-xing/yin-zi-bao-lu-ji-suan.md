...
**缺失值处理**：风险模型是由不同的风险因子为基础构成的，我们希望在截面回归的过程中保障每一只股票的因子值都是完整的，这保障了不同因子的回归检测结果的可比性
...
我们认为行业、市值或者其他属性相似的股票会有相似的因子暴露。所以我们希望用行业的均值（中位数）来填充缺失值。但我们仍然认为均值（中位数）并不是一个特别好的方法，因为。
